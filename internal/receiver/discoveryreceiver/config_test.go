...
func TestInvalidConfigs(t *testing.T) {
	tests := []struct{ name, expectedError string }{
		...
		{name: "reserved_receiver_name", expectedError: `Received unexpected error: decoding failed due to the following error(s): error decoding 'receivers': in "with{endpoint=}/" id: invalid character(s) in name "with{endpoint=}/"`},
	}
